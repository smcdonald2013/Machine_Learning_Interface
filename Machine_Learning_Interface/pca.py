...
class PCA(DimensionalityReduction):
    ...
    **kwargs : varies
    ...
    def diagnostics(self):
        ...
        scikit_mixin.biplot(self.x_train, self.evals, self.evecs, var_names=self.x_val.columns, var=self.kernel)
    ...
    def transform(self, x_val):
        ...
        val_df   = pd.DataFrame(index=self.x_val.index, data=val_pred, columns=factor_columns)  ###TO DO: Title Factors
        return val_df
    ...
    def scree_plot(self, evals):
        """Plots scree plot- eigenvalues associated with each principal component.
